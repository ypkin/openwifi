...
static void ac_wait_terminate_allsessions(void) {
	...
	while (g_ac.sessionsthread->count > 0) {
		log_printf(LOG_DEBUG, "Waiting for %lu session terminate", g_ac.sessionsthread->count);
...
static void signal_cb (EV_P_ ev_signal *w, int revents)
{
	...
	ev_io_stop(EV_DEFAULT_UC_ &g_ac.socket_ev);
...
static void ac_dfa_process_packet(void *buffer, int buffersize,
				   ...
				   int sock)
{
	...
	if (session) {
		...
		if (sessioncount < g_ac.descriptor.maxwtp) {
			int check = capwap_sanity_check(CAPWAP_UNDEF_STATE, buffer, buffersize, g_ac.enabledtls);
				...
				if (!IS_FLAG_F_HEADER(header)) {
					...
					if (buffersize >= (headersize + sizeof(struct capwap_control_message))) {
						...
						if (type == CAPWAP_DISCOVERY_REQUEST) {
							ac_discovery_add_packet(buffer, buffersize, sock, fromaddr);
						} else if (!g_ac.enabledtls && (type == CAPWAP_JOIN_REQUEST)) {
							/* Create a new session */
							session = ac_create_session(sock, fromaddr, toaddr);
...
static void capwap_control_cb(EV_P_ ev_io *w, int revents)
{
	...
	do {
		do {
			...
			r = capwap_recvfrom(w->fd, buffer, sizeof(buffer), &fromaddr, &toaddr);
		} while (r < 0 && errno == EINTR);
		log_printf(LOG_DEBUG, "AC got data: r: %zd", r);

		if (!g_ac.running) {
			log_printf(LOG_DEBUG, "Closing AC, Teardown connection");
		...
		ac_dfa_process_packet(&buffer, r, &fromaddr, &toaddr, w->fd);
	} while (ev_is_active(w));
}


static void ac_socket_io_start()
{
	log_printf(LOG_DEBUG, "Start EV_IO on socket %d", g_ac.net.socket);

	/* Configure libev struct */
	ev_io_init (&g_ac.socket_ev, capwap_control_cb, g_ac.net.socket, EV_READ);
	ev_io_start(EV_DEFAULT_UC_ &g_ac.socket_ev);
}

static void ac_socket_io_stop()
{
	log_printf(LOG_DEBUG, "Stop EV_IO on socket %d", g_ac.socket_ev.fd);
...
int ac_execute(void) {
	...
	ev_signal_init(&g_ac.sigint_ev, signal_cb, SIGINT);
	ev_signal_init(&g_ac.sigterm_ev, signal_cb, SIGTERM);
	ev_signal_start(EV_DEFAULT_UC_ &g_ac.sigint_ev);
	ev_signal_start(EV_DEFAULT_UC_ &g_ac.sigterm_ev);
