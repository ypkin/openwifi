...
void usage(void)
{
	fprintf(stderr, "Usage: WTP [OPTION]\n");
	fprintf(stderr, "	-f			Foreground\n");
	fprintf(stderr, "	-c /etc/ac	Specify conf directory\n");
...
int main (int argc, const char * argv[])
{
	int c;
	int run_daemon = 1;
	char *confdir = "/etc/wtp";
	
	while (-1 != (c = getopt(argc, argv, "hfdc:"))) {
		switch(c) {
		case 'h':
			usage();
			exit(1);
